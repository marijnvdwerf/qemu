...
#define LCD(obj) OBJECT_CHECK(lcd_state, (obj), TYPE_LCD)
...
{
    return ((val * 0x0802LU & 0x22110LU) | (val * 0x8020LU & 0x88440LU)) *
               0x10101LU >>
           16;
}

static uint32_t parse_header(lcd_state *s, uint16_t header)
{
...
static uint32_t jdi_lpm013m126c_transfer(SSIPeripheral *bus, uint32_t val32)
{
    lcd_state *s = LCD(bus);
...
static void sm_lcd_update_display(void *arg)
{
    ...
    for (int y = 0; y < 176; y++) {
        for (int x = 0; x < 176; x++) {
            ...
            *((uint32_t *)d) =
                rgb_to_pixel32(s->buffer[y][x].r * 255, s->buffer[y][x].g * 255,
                               s->buffer[y][x].b * 255);
            d += 4;
        }
    }
...
static void sm_lcd_invalidate_display(void *arg) { lcd_state *s = arg; }
...
static void jdi_lpm013m126c_realize(struct SSIPeripheral *d, Error **errp)
{
