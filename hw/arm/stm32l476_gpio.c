...
static void stm32l476_gpio_reset(DeviceState *dev)
{
    STM32L476GpioState *s = STM32L476_GPIO(dev);
}

static uint64_t stm32l476_gpio_read(void *opaque, hwaddr offset,
                                    unsigned int size)
{
...
static void stm32l476_gpio_write(void *opaque, hwaddr offset, uint64_t val64,
                                 unsigned int size)
{
    STM32L476GpioState *s = opaque;
    ...
    case GPIOx_BSRR: {
        /*
         * If there is an attempt to both set and reset a bit in GPIOx_BSRR,
    ...
    for (int i = 0; i < 16; i++) {
        ...
        if (false) {
            x = &backup[i];
            printf("[mode:%d AFSEL:%d ospeed:%d ot:%d pupd:%d]", x->mode,
                   x->AFSEL, x->OSPEED, x->OT, x->PUPD);
            printf(" to ");
        }
        x = &s->pins[i];
...
static void stm32l476_gpio_init(Object *obj)
{
...
static void stm32l476_gpio_class_init(ObjectClass *klass, void *data)
{
...
static void stm32l476_gpio_register_types(void)
{
