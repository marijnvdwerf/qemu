...
/*
 * NOTE: The usleep() helps the MacOS stdout from freezing when we have a lot of
 * print out
 */
#define DPRINTF(fmt, ...)                                                      \
    do {                                                                       \
        qemu_log_mask(LOG_GUEST_ERROR, "LPTIM %s: " fmt, __func__,             \
                      ##__VA_ARGS__);                                          \
        usleep(100);                                                           \
    } while (0)
...
static int64_t stm32l47xx_lptim_next_transition(STM32L476LPTimState *s,
                                     ...
                                     unsigned int size)
{
    ...
    case LPTIM_CNT: {
        ...
        uint64_t kI =
            ptimer_get_limit(s->ptimer) - ptimer_get_count(s->ptimer) - 1;
...
static void stm32l476_lptim_write(void *opaque, hwaddr offset, uint64_t val64,
                                  unsigned int size)
{
    STM32L476LPTimState *s = opaque;
    ...
    case LPTIM_IER: {
        if (s->ENABLE) {
            qemu_log_mask(LOG_GUEST_ERROR,
                          "The LPTIM_IER register must only be modified when "
                          "the LPTIM is disabled");
            return;
        }
        s->CMPMIE = (val64 >> 0) & 1;
        s->ARRMIE = (val64 >> 1) & 1;
...
static void stm32l476_lptim_realize(DeviceState *dev, Error **errp)
{
    ...
    if (false) {
        s->timer =
            timer_new_ns(QEMU_CLOCK_VIRTUAL, stm32l476_lptim_interrupt, s);
    }
