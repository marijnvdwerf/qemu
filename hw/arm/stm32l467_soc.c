...
static inline void create_unimplemented_layer(const char *name, hwaddr base,
                                              hwaddr size)
{
...
static void stm32l467_soc_initfn(Object *obj)
{
    ...
    object_initialize_child(obj, "pwr", &s->pwr, TYPE_STM32L476_PWR);

    object_initialize_child(obj, "lptim1", &s->lptim1, TYPE_STM32L476_LPTIM);

    object_initialize_child(obj, "rcc", &s->rcc, TYPE_STM32L476_RCC);

    object_initialize_child(obj, "flash", &s->flash_r, TYPE_STM32L476_FLASH);

    object_initialize_child(obj, "DMA2", &s->dma, TYPE_STM32L476_DMA);

    object_initialize_child(obj, "QSPI", &s->qspi, TYPE_STM32L476_QSPI);

    for (int i = 0; i < 3; i++) {
        object_initialize_child(obj, "I2C[*]", &s->i2c[i], TYPE_STM32L476_I2C);
...
static void stm32l467_soc_realize(DeviceState *dev_soc, Error **errp)
{
    ...
    s->syscfg = qdev_new("stm32l476-syscfg");
    ...
    for (int i = 0; i < ARRAY_SIZE(gpio_desc); ++i) {
        s->gpio[i] = qdev_new(TYPE_STM32L476_GPIO);
    ...
    memory_region_init_alias(&s->flash_alias, OBJECT(dev_soc), "alias",
    ...
    memory_region_init_ram(&s->sram1, OBJECT(dev_soc), "STM32F205.sram1",
                           96 * 1024, &error_fatal);
    memory_region_add_subregion(system_memory, 0x20000000,
                                &s->sram1); /* 0x18000 */
    /*  memsave 0x20000000 0x18000 ../sram1.bin */

    memory_region_init_ram(&s->sram2, OBJECT(dev_soc), "STM32F205.sram2",
                           32 * 1024, &error_fatal);
    memory_region_add_subregion(system_memory, 0x10000000,
                                &s->sram2); /* 0x8000 */
