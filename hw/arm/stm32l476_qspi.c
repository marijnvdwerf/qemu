...
static StartMode getStart(STM32L476QspiState *s)
{
...
static void do_transfer(STM32L476QspiState *s)
{
    /* Instruction phase */
    qemu_irq_lower(s->cs);
...
static void start(STM32L476QspiState *s)
{
...
static void stm32l476_quadspi_reset(DeviceState *dev)
{
    STM32L476QspiState *s = STM32L476_QSPI(dev);
}

static uint64_t stm32l476_quadspi_read(void *opaque, hwaddr offset,
                                       unsigned int size)
{
    ...
    switch (offset) {
    case QUADSPI_CR:
        DPRINTF("QSPI: read CR\n");
        return 0;

    case QUADSPI_DCR:
        DPRINTF("QSPI: read DCR\n");
        return 0;
...
static void stm32l476_quadspi_write(void *opaque, hwaddr offset, uint64_t val64,
                                    unsigned int size)
{
    STM32L476QspiState *s = opaque;
    StartMode kStart;
    ...
    case QUADSPI_DCR:
        DPRINTF("QSPI: DCR = 0x%08X\n", val64);
        break;

    case QUADSPI_FCR:
        DPRINTF("QSPI: FCR = 0x%08X\n", val64);
        if (val64 & (1 << 0)) {
            s->TEF = false;
        }
        if (val64 & (1 << 1)) {
    ...
    case QUADSPI_DLR:
        s->DL = extract64(val64, 0, 32);
        break;

    case QUADSPI_CCR:
        s->INSTRUCTION = extract64(val64, 0, 8);
    ...
    case QUADSPI_AR:
        s->ADDRESS = extract64(val64, 0, 32);
        kStart = getStart(s);
        if (kStart == OnWriteAR) {
            start(s);
        }
...
static uint64_t cache_read(void *opaque, hwaddr offset, unsigned int size)
{
    STM32L476QspiState *s = opaque;
...
static void cache_write(void *opaque, hwaddr offset, uint64_t val64,
                        unsigned int size)
{
...
static void stm32l476_quadspi_init(DeviceState *dev, Error **pError)
{
    STM32L476QspiState *s = STM32L476_QSPI(dev);
    ...
    memory_region_init_io(&s->mmio, dev, &stm32l476_quadspi_ops, s, "QPSI_R",
                          0x400);
...
static void stm32l476_quadspi_class_init(ObjectClass *klass, void *data)
{
...
static void stm32l476_quadspi_register_types(void)
{
